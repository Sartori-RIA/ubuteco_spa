...
export class DrinkEffects {
  ...
  requestAllDrinks$ = createEffect(() => this.actions$.pipe(
    ...
    mergeMap(() => this.drinkService.all()),
    map((drinks: Drink[]) => REQUEST_ALL_DRINKS_DONE({drinks})),
    catchError(() => {
  ...
  createDrink$ = createEffect(() => this.actions$.pipe(
    ofType(CREATE_DRINK),
    mergeMap((action) => this.drinkService.create(action.drink)),
    map((drink: Drink) => {
  ...
  requestBeerById$ = createEffect(() => this.actions$.pipe(
    ...
    map((drink: Drink) => REQUEST_DRINK_DONE({drink})),
    catchError(() => {
